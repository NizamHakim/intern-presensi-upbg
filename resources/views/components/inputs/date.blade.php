<div data-default="{{ $value }}" {{ $attributes->merge(['class' => 'flex flex-col w-full input-group input-date ']) }}>
    ...
    <div class="relative">
        <input name="{{ $inputName }}" value="{{ $value }}" placeholder="{{ $placeholder }}" class="w-full pl-3 pr-8 py-2 h-10 rounded-md bg-gray-200 text-gray-600 font-medium placeholder:text-gray-400 outline outline-transparent outline-1.5 outline-offset-0 transition-all focus:outline-upbg-light cursor-pointer">
