<div data-default="{{ ($value) ?  $value->isoFormat("HH:mm") : null }}" class="flex flex-col w-full input-group input-time">
    ...
    <div class="relative">
        <input name="{{ $inputName }}" value="{{ ($value) ? $value->isoFormat("HH:mm") : null }}" placeholder="{{ $placeholder }}" {{ $attributes->merge(['class' => 'w-full pl-3 pr-8 py-2 h-10 rounded-md bg-gray-200 text-gray-600 font-medium placeholder:text-gray-400 outline outline-transparent outline-1.5 outline-offset-0 transition-all focus:outline-upbg-light cursor-pointer']) }}>
