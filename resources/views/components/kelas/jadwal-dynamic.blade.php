<section id="jadwal-section" class="flex flex-col gap-2">
    ...
    <div class="jadwal-container flex flex-col gap-3">
            ...
            @foreach ($kelas->jadwal as $jadwal)
                <div class="jadwal-item flex flex-row items-center border shadow-sm rounded-md md:border-none md:shadow-none md:items-start">
                    ...
                    <div class="jadwal-buttons flex flex-row items-center h-fit md:items-start @if (!$loop->first) md:ml-3 @endif">
                        ...
                        @if (!$loop->first)
                            <button type="button" class="delete-jadwal font-medium text-red-600 bg-white size-8 rounded-sm-md transition md:hover:bg-red-600 md:hover:text-white md:hover:border-red-600 md:size-10 md:rounded-full md:border md:shadow-sm"><i class="fa-regular fa-trash-can"></i></button>
                        @endif
                    </div>
                </div>
            @endforeach
        @else
            <div class="jadwal-item flex flex-row items-center border shadow-sm rounded-md md:border-none md:shadow-none md:items-start">
                <div class="input-container flex flex-row items-center flex-1 pl-3 py-2 gap-2 md:p-0 md:items-start md:mr-[3.25rem]">
                    <div class="input-group md:flex-1">
                        <x-inputs.dropdown.select name="hari" placeholder="Pilih hari" class="hari-dropdown hidden md:block">
                            @foreach ($hariOptions as $hari)
                                <x-inputs.dropdown.option :value="$hari['value']">{{ $hari['text'] }}</x-inputs.dropdown.option>
                            @endforeach
                        </x-inputs.dropdown.select>
                        <p class="text-gray-700 md:hidden"><i class="fa-solid fa-calendar-days mr-2"></i><span class="hari-mobile">Hari,</span></p>
                    </div>
                    <div class="flex flex-row items-center gap-2 md:flex-2 md:items-start">
                        <div class="input-group min-w-0 md:flex-1">
                            <x-inputs.time inputName="waktu-mulai" placeholder="Waktu mulai" class="waktu-mulai hidden md:block"></x-inputs.time>
                            <p class="waktu-mulai-mobile text-gray-700 md:hidden">Mulai</p>
                        </div>
                        <span class="text-gray-700 text-lg md:mt-2">-</span>
                        <div class="input-group min-w-0 md:flex-1">
                            <x-inputs.time inputName="waktu-selesai" placeholder="Waktu selesai" class="waktu-selesai hidden md:block"></x-inputs.time>
                            <p class="waktu-selesai-mobile text-gray-700 md:hidden">Selesai</p>
                        </div>
                    </div>
                </div>
                <div class="jadwal-buttons flex flex-row items-center h-fit md:items-start">
                    <button type="button" class="edit-jadwal font-medium text-upbg bg-white size-8 rounded-sm-md transition md:hidden"><i class="fa-regular fa-pen-to-square"></i></button>
                </div>
            </div>
        @endif
    ...
    <div class="w-full flex flex-row justify-center items-center gap-4 mt-1">
        <button type="button" class="tambah-jadwal font-medium text-gray-600 size-10 border shadow-sm rounded-full text-lg transition hover:text-white hover:bg-green-600 hover:border-green-600"><i class="fa-solid fa-plus"></i></button>
    </div>
</section>
